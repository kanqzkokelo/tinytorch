...
int main(void) {
    printf("=========================================================================\n");
    printf("  Testing Q8_0 KV Cache Scatter & Flash Attention Correctness\n");
    printf("=========================================================================\n");
    ...
    CK(cudaMalloc((void **)&d_Kc_q8, q8_cache_size));
    CK(cudaMalloc((void **)&d_Vc_q8, q8_cache_size));

    // Workspace for Split-K Q8_0
    int max_S = 16;
    ...
    for (int ti = 0; ti < num_tests; ti++) {
        ...
        CK(cudaMemset(d_Kc_q8, 0, q8_cache_size));
        CK(cudaMemset(d_Vc_q8, 0, q8_cache_size));

        // Scatter N slots into FP32 and Q8_0 caches
        for (int t = 0; t < N; t++) {
            ...
            tt_kv_scatter_q8_0(d_kst, d_vst, d_Kc_q8, d_Vc_q8, d_pos, n_kv_heads, head_dim, max_ctx, 0);
        }
        ...
        // Q8_0 flash attention
        tt_flash_gqa_q8_0(d_q, d_Kc_q8, d_Vc_q8, d_out_q8, d_pos,
                          n_heads, n_kv_heads, head_dim, max_ctx, scale, window, 0);
        ...
        for (size_t i = 0; i < (size_t)n_heads * head_dim; i++) {
            ...
            float q8_val = h_out_q8[i];

            if (isnan(fp_val) || isinf(fp_val) || isnan(q8_val) || isinf(q8_val)) {
                nan_inf_count++;
            }

            float err = fabsf(fp_val - q8_val);
            if (err > max_abs_err) max_abs_err = err;
        }

        // Also test Split-K Q8_0
        int S = 4;
        ...
        printf("N = %4d | max_abs_err (standard): %11.4e | max_abs_err (split-k S=%d): %11.4e | nan/inf: %d | %s\n",
               ...
               (max_abs_err < 1e-2f && max_abs_err_splitk < 1e-2f && nan_inf_count == 0) ? "PASS" : "FAIL");

        if (max_abs_err >= 1e-2f || max_abs_err_splitk >= 1e-2f || nan_inf_count > 0) {
            total_failures++;
        }
    }
