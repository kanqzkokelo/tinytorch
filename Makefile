...
$(BUILD)/run_llm_gpu: examples/run_llm_gpu.c src/loader_gguf.c src/tokenizer_bpe.c src/async_printer.c kernels/gemv_q4_cuda.cu | $(BUILD)
	$(NVCC) -O3 -gencode arch=compute_86,code=sm_86 \
	  -I$(CUDA_INC) -Iinclude -Isrc -Xcompiler -fPIC \
	  -Xlinker -rpath=$(CURDIR)/build:$(HOME)/mmcuda/lib \
	  -o $@ \
	  examples/run_llm_gpu.c src/loader_gguf.c src/tokenizer_bpe.c src/async_printer.c kernels/gemv_q4_cuda.cu \
	  -L$(HOME)/mmcuda/lib -lcudart -lpthread

run_llm_gpu: $(BUILD)/run_llm_gpu

$(BUILD)/chat_llm_gpu: examples/chat_llm_gpu.c src/loader_gguf.c src/tokenizer_bpe.c src/async_printer.c kernels/gemv_q4_cuda.cu | $(BUILD)
	$(NVCC) -O3 -gencode arch=compute_86,code=sm_86 \
	  -I$(CUDA_INC) -Iinclude -Isrc -Xcompiler -fPIC \
	  -Xlinker -rpath=$(CURDIR)/build:$(HOME)/mmcuda/lib \
	  -o $@ \
	  examples/chat_llm_gpu.c src/loader_gguf.c src/tokenizer_bpe.c src/async_printer.c kernels/gemv_q4_cuda.cu \
	  -L$(HOME)/mmcuda/lib -lcudart -lpthread
